...
function MultiFileCodeNodeView({
  node,
  updateAttributes,
  deleteNode,
  selected,
  ...
  const [activeTab, setActiveTab] = useState(
    node.attrs.activeFileId || files[0]?.id
  );
  ...
  return (
    <NodeViewWrapper className="my-4">
      {/* macOS-style Window */}
      <div
        className={`rounded-xl overflow-hidden transition-all border border-zinc-700/50 ${
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <X
                size={7}
                className="text-red-900/80 opacity-0 group-hover:opacity-100"
              />
            </button>
            <div
          ...
          <div className="flex-1 flex items-center gap-0.5 mx-2 overflow-x-auto">
            {files.map((file) => (
              <div
                ...
                className={`group flex items-center gap-1.5 px-3 py-1 rounded-md text-xs cursor-pointer transition-all ${
                  activeTab === file.id
        ...
        {activeFile && (
          <div className="relative min-h-[180px] overflow-hidden bg-[#1a1a1a]">
            ...
            <textarea
              ...
              onChange={(e) =>
                handleUpdateFile(activeFile.id, { code: e.target.value })
              }
...
export const CustomMultiFileCode = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      'div',
      mergeAttributes(HTMLAttributes, { 'data-multi-file-code': '' }),
    ];
  },
