...
function checkGeminiRateLimit(functionName: string): boolean {
  ...
  geminiRequestCount++;
  console.log(
    `📊 Gemini requests this minute: ${geminiRequestCount}/${GEMINI_MAX_REQUESTS_PER_MINUTE} (${functionName})`
  );
  return true;
}
...
export async function generateImage(
  ...
  if (!checkGeminiRateLimit('generateImage')) {
    throw new Error(
      'Rate limit reached for image generation. Please try again in a minute.'
    );
  }
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (candidate?.content?.parts?.[0]?.inlineData) {
          ...
          // Cache the result
          imageGenerationCache.set(prompt, {
            url: dataUrl,
            timestamp: Date.now(),
          });
        ...
        console.warn(
          `⚠️ Image generation attempt ${attempt + 1} failed:`,
          lastError.message
        );
...
export async function editImage(
  ...
  if (!checkGeminiRateLimit('editImage')) {
    throw new Error(
      'Rate limit reached for image editing. Please try again in a minute.'
    );
  }
...
export async function recommendCourses(
  ...
  if (!checkGeminiRateLimit('recommendCourses')) {
    ...
    return JSON.stringify({
      courseId: 'cs-101',
      reason:
        'Rate limit reached. Recommending our popular introductory course!',
    });
  }
