  ...
  if (!src || error) {
    return (
      <div className="w-24 h-24 rounded-lg bg-gradient-to-br from-blue-500/20 to-indigo-500/20 flex items-center justify-center">
        <span className="text-2xl font-bold text-blue-500/60">
...
export default function AdminDashboard() {
  ...
  const [recentUsers, setRecentUsers] = useState<RecentUser[]>(
    () =>
      dashboardCache.getCached<{ users: RecentUser[] }>(CACHE_KEYS.USERS)
        ?.users || []
  );
  ...
  const hasCachedData =
    courses.length > 0 || stats !== null || recentUsers.length > 0;
  ...
  const fetchUsers = useCallback(
    () =>
      fetch('/api/admin/users?limit=5&page=1').then((r) =>
        r.ok ? r.json() : { users: [] }
      ),
    []
  );
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const [coursesData, statsData, usersData, featuresData] =
          await Promise.all([
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500 dark:text-neutral-400">
                  Feature flags, system status & settings
                </p>
              </div>
            </div>
      ...
      <div className="space-y-4 animate-in fade-in duration-500 delay-600">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {courses.map((course) => (
            <Card
              key={course.id}
              className="p-4 bg-white dark:bg-white/5 border-neutral-200 dark:border-white/10 hover:bg-neutral-50 dark:hover:bg-white/10 transition-all group"
            >
