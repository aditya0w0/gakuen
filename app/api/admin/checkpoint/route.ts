 ...
 * Body: Course JSON (optionally gzip compressed)
 *
...
async function parseRequestBody(request: NextRequest): Promise<any> {
  ...
  if (contentType === 'application/gzip' || contentEncoding === 'gzip') {
    ...
    console.log(
      `📦 Received compressed payload: ${(buffer.length / 1024).toFixed(1)}KB`
    );

    try {
      const decompressed = gunzipSync(buffer);
      console.log(
        `📦 Decompressed to: ${(decompressed.length / 1024).toFixed(1)}KB`
      );
...
export async function POST(request: NextRequest) {
  ...
  try {
    const course = await parseRequestBody(request);
    ...
    try {
      ...
      if (firestoreError?.message === 'Firestore timeout') {
        console.warn(
          `⚠️ [Firestore] Timeout after 5s - Telegram save succeeded`
        );
      } else if (
        ...
        console.warn(
          `⚠️ [Firestore] Quota exhausted - Telegram save succeeded`
        );
      } else {
