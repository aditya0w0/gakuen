        ...
        if (contentType.includes('application/json')) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    return NextResponse.json({ error: `Failed to fetch video: ${response.status}` }, { status: 400 });
                }

                const contentType = response.headers.get('content-type') || '';
                if (!contentType.startsWith('video/')) {
                    return NextResponse.json({ error: 'URL does not point to a video' }, { status: 400 });
                }
                ...
                console.log(`✅ Fetched video: ${buffer.length} bytes, type: ${contentType}`);
                ...
                if (isDriveEnabled()) {
                    const result = await uploadToDrive(buffer, `video-${uuidv4()}.${ext}`, 'cms', contentType);
                    if (result.url) {
                ...
                return NextResponse.json({ error: 'Failed to fetch external video' }, { status: 400 });
            }
        }
